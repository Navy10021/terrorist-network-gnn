...
class NetworkAugmenter:
    ...
    def add_gaussian_noise(
        self, node_features: torch.Tensor, noise_std: float = 0.01
    ) -> torch.Tensor:
    ...
    def temporal_interpolation(
        self, network: MultiLayerTemporalNetwork, num_interpolations: int = 1
        ...
        for t in range(len(network.layers_history) - 1):
            ...
            for i in range(1, num_interpolations + 1):
                ...
                interpolated_layers = self._interpolate_layers(layers_t, layers_t1, alpha)
    ...
    def _interpolate_layers(
        self, layers_a: List[NetworkLayer], layers_b: List[NetworkLayer], alpha: float
    ) -> List[NetworkLayer]:
        ...
        for layer_a, layer_b in zip(layers_a, layers_b):
            ...
            edge_weights = torch.cat(
                [layer_a.edge_weights * (1 - alpha), layer_b.edge_weights * alpha]
            )
    ...
    def augment_network(
        ...
        augmented_network = MultiLayerTemporalNetwork(
            num_nodes=network.num_nodes, num_layers=network.num_layers
        )
                    ...
                    for i in range(aug_edge_index.size(1)):
                        ...
                        for j in range(layer.edge_index.size(1)):
                            if torch.equal(aug_edge_index[:, i], layer.edge_index[:, j]):
                                ...
                                break
                    aug_edge_features = (
                        layer.edge_features[mask]
                        if mask.any()
                        else layer.edge_features[: aug_edge_index.size(1)]
                    )
                    aug_edge_weights = (
                        layer.edge_weights[mask]
                        if mask.any()
                        else layer.edge_weights[: aug_edge_index.size(1)]
                    )
                else:
                    aug_edge_features = layer.edge_features
                    aug_edge_weights = layer.edge_weights

                augmented_layers.append(
                    NetworkLayer(
                        name=layer.name,
...
class TerroristNetworkGenerator:
    ...
    def generate_temporal_network(
        ...
        network = MultiLayerTemporalNetwork(num_nodes=self.config.max_nodes, num_layers=5)

        self._initialize_network()

        for t in range(num_timesteps):
            layers = self._generate_timestep(t, num_node_features, num_edge_features, device)
        ...
        for i, node_type in enumerate(self.node_types):
            attr = {
                ...
                "trust_score": (
                    ...
                    if node_type != "supporter"
                    else self.rng.uniform(0.2, 0.6)
                ),
    ...
    def _generate_timestep(
        self, t: int, num_node_features: int, num_edge_features: int, device: torch.device
    ) -> List[NetworkLayer]:
        """Generate all 5 layers for timestep t"""
        num_nodes = len(self.node_types)

        node_features = self._generate_node_features(num_nodes, num_node_features, device)
        ...
        digital = self._generate_digital_layer(num_nodes, num_edge_features, node_features, device)
    ...
    def _generate_node_features(
        self, num_nodes: int, num_features: int, device: torch.device
    ) -> torch.Tensor:
                ...
                if len(edges) < num_nodes:
                    ...
                    if hierarchy_respect:
                        same_level = [
                            ...
                            if j != i
                            and abs(self.hierarchy_level[j] - self.hierarchy_level[i]) <= 1
                        ]
                        if same_level:
    ...
    def _generate_financial_layer(
        ...
        for (i, j), w in zip(edges, weights):
            if self.node_attributes[i]["financial_capacity"] > 0.3:
                ...
                filtered_weights.append(w * self.node_attributes[i]["financial_capacity"])
    ...
    def _generate_operational_layer(
        ...
        operative_indices = [
            i for i, nt in enumerate(self.node_types) if nt in ["operative", "leader"]
        ]
    ...
    def _evolve_network(self):
        ...
        if num_dropout > 0:
            dropout_probs = np.array(
                [
                    0.05 if nt == "leader" else 0.10 if nt == "operative" else 0.30
...
class DisruptionEvaluator:
    ...
    def evaluate_disruption_strategy(
        self, network: MultiLayerTemporalNetwork, removed_nodes: List[int], timestep: int = -1
    ) -> Dict[str, float]:
        ...
        for layer in layers:
            ...
            layer_impact = self._compute_layer_impact(layer, set(removed_nodes))
        ...
        resilience_metrics = self.compute_network_resilience(network, removed_nodes, timestep)
    ...
    def _compute_layer_impact(self, layer: NetworkLayer, removed_nodes: Set[int]) -> float:
    ...
    def _compute_operational_capacity(
        self, operational_layer: NetworkLayer, removed_nodes: Set[int]
    ) -> float:
    ...
    def compute_network_resilience(
        ...
        structural_resilience = self._compute_path_redundancy(layers, removed_set)

        # 2. Functional resilience
        functional_resilience = self._compute_functional_capacity(layers, removed_set)

        # 3. Adaptation capacity
        adaptation_capacity = self._compute_adaptation_potential(layers, removed_set)
        ...
        return {
            ...
            "overall_resilience": (
                structural_resilience + functional_resilience + adaptation_capacity
            )
            / 3,
        }

    def _compute_path_redundancy(
        self, layers: List[NetworkLayer], removed_nodes: Set[int]
    ) -> float:
