...
class AblationStudy:
    ...
    def _simplify_graph_layers(self, model):
        ...
        if hasattr(model, 'layer_gnns'):
            ...
            for idx, layer_gnn in enumerate(model.layer_gnns):
                ...
                print(f"    DEBUG: layer_gnn[{idx}] hasattr graph_layers = {hasattr(layer_gnn, 'graph_layers')}")
                ...
                if hasattr(layer_gnn, 'graph_layers'):
                    ...
                    print(f"    DEBUG: Replacing graph_layers with SimpleGCNLayer, hidden_dim={hidden_dim}")
                    num_layers = len(layer_gnn.graph_layers)
                    layer_gnn.graph_layers = nn.ModuleList([
                        SimpleGCNLayer(hidden_dim)
        ...
        if hasattr(model, 'layer_gnns'):
            ...
            def static_forward(multi_layer_network, timestamps):
                ...
                static_network = MultiLayerTemporalNetwork(
                    multi_layer_network.num_nodes,
                    multi_layer_network.num_layers
                )
            ...
            def static_forward(node_features_seq, edge_indices_seq, edge_features_seq, timestamps, batch_seq=None):
    ...
    def run_ablation_study(
        ...
        full_results = self._evaluate_model(
            self.full_model, networks, evaluator, detector, top_k
        )
        results['full_model'] = {
            'name': 'Full Model',
        ...
        print(f"  Mean Disruption: {results['full_model']['mean_disruption']:.4f} "
        ...
        for variant_key, variant_info in self.variants.items():
            ...
            try:
                ...
                print(f"  Mean Disruption: {results[variant_key]['mean_disruption']:.4f} "
        ...
        for i, (variant_key, variant_data) in enumerate(ranked_variants, 1):
            ...
            print(f"   Performance Drop: {variant_data['performance_drop']:.4f} "
...
class ZeroTimeEncoder(nn.Module):
    ...
    def forward(self, timestamps):
        if timestamps.dim() == 1:
            ...
            return torch.zeros(timestamps.size(0), timestamps.size(1), self.d_model, device=timestamps.device)
