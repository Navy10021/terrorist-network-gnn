...
class HierarchicalTemporalPooling(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_scales: int = 3):
        ...
        self.scales = nn.ModuleList([
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=2**i, stride=2**i, padding=2**(i-1) if i > 0 else 0)
...
class EnhancedTemporalMemoryBank(nn.Module):
    ...
    def __init__(self, memory_size: int, memory_dim: int, num_heads: int = 4):
        ...
        self.update_gate = nn.Sequential(
            nn.Linear(memory_dim * 3, memory_dim),  # query, old_memory, retrieved
            nn.Sigmoid()
        )
    ...
    def _update_memory(self, query: torch.Tensor, retrieved: torch.Tensor, 
