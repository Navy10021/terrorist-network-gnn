...
class StatisticalAnalyzer:
    ...
    def wilcoxon_test(
        ...
        if significant:
            if np.median(method_a) > np.median(method_b):
                interpretation = f"Method A significantly better (p={p_value:.4f}, rank-biserial r={r:.3f})"
            else:
                interpretation = f"Method B significantly better (p={p_value:.4f}, rank-biserial r={r:.3f})"
        else:
            interpretation = f"No significant difference (p={p_value:.4f})"
    ...
    def mann_whitney_test(
        ...
        statistic, p_value = stats.mannwhitneyu(
            method_a, method_b, alternative=alternative
        )
    ...
    def compare_multiple_methods(
        ...
        if len(p_values) > 0:
            ...
            for i, method_name in enumerate([m for m in results_dict.keys() if m != reference_method]):
...
class ResultVisualizer:
    ...
    def plot_pairwise_comparison(
        ...
        # Effect sizes
        colors = ['green' if abs(d) > 0.5 else 'orange' if abs(d) > 0.2 else 'gray' 
