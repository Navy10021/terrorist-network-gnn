"""
Complete Experimental Pipeline - Advanced Implementation
=========================================================
...
class TemporalGNNExperiment:
    """
    Complete experimental pipeline for Temporal GNN network disruption
    """

    def __init__(
        self, config: NetworkConfig, model_config: Dict, output_dir: str = "experiments/results"
    ):
        self.config = config
        ...
        self.detector = EnhancedCriticalNodeDetector()
    ...
    def run_complete_experiment(
        ...
        """
        print("=" * 80)
        print("TERRORIST NETWORK DISRUPTION - TEMPORAL GNN EXPERIMENT")
        print("=" * 80)
        print(f"\nConfiguration:")
    ...
    def _build_and_train_model(self, train_networks, val_networks) -> nn.Module:
        ...
        trainer = EnhancedTemporalGNNTrainer(
            model, learning_rate=1e-3, weight_decay=1e-5, device=self.device
        )
        ...
        history = trainer.fit(
            train_networks, val_networks, num_epochs=50, early_stopping_patience=10, verbose=True
        )
    ...
    def _compare_baselines(self, test_networks, our_model):
        ...
        baseline_eval.add_gnn_baseline(
            "static_gcn",
            StaticGCN(self.model_config["num_node_features"], self.model_config["hidden_dim"]),
        )

        print("  Adding Static GAT...")
        baseline_eval.add_gnn_baseline(
            "static_gat",
            StaticGAT(self.model_config["num_node_features"], self.model_config["hidden_dim"]),
        )
        ...
        baseline_eval.add_gnn_baseline(
            "static_graphsage",
            StaticGraphSAGE(
                self.model_config["num_node_features"], self.model_config["hidden_dim"]
            ),
        )
        ...
        for network_idx, network in enumerate(test_networks):
            ...
            results = baseline_eval.compare_all_baselines(network, self.evaluator, top_k=10)
        ...
        with torch.no_grad():
            for network in test_networks:
                ...
                critical_nodes, _ = self.detector.detect_critical_nodes(
                    agg_edge_index.to(self.device), num_nodes, embeddings, top_k=10
                )
        ...
        for method_name in sorted(
            all_results.keys(), key=lambda x: np.mean(all_results[x]), reverse=True
        ):
            ...
            print(
                f"{method_name:<25}{marker} {mean:<12.4f} {std:<12.4f} {min_val:<12.4f} {max_val:<12.4f}"
            )
        ...
        with torch.no_grad():
            for net_idx, network in enumerate(networks):
                ...
                for strategy_name, strategy_config in strategies.items():
                    detector = EnhancedCriticalNodeDetector(**strategy_config)
    ...
    def _experiment_q2_temporal_prediction(self, networks, model):
        ...
        hidden_dim = (
            model.output_dim if hasattr(model, "output_dim") else self.model_config["hidden_dim"]
        )
        ...
        with torch.no_grad():
            for net_idx, network in enumerate(networks):
                ...
                embeddings = model(network, timestamps)
                critical_nodes, _ = self.detector.detect_critical_nodes(
                    agg_edge_index.to(self.device), num_nodes, embeddings, top_k=5
                )
                ...
                results.append(
                    {
                        ...
                        "edge_probs_mean": (
                            resilience_result["edge_probabilities"].mean().item()
                            if resilience_result["edge_probabilities"].numel() > 0
                            else 0.0
                        ),
                        "recruitment_probs_mean": resilience_result["recruitment_probabilities"]
                        .mean()
                        .item(),
                    }
                )
            ...
            for net_idx, network in enumerate(networks):
                ...
                for strategy in adaptation_strategies:
                    ...
                    for round_num in range(1, 4):
                        ...
                        current_num_nodes = (
                            new_num_nodes.item()
                            if torch.is_tensor(new_num_nodes)
                            else new_num_nodes
                        )
    ...
    def _ablation_study(self, test_networks, model):
        ...
        ablation_results = ablation.run_ablation_study(
            test_networks, self.evaluator, self.detector, top_k=10
        )
    ...
    def _save_and_visualize_results(self):
        ...
        if (
            "statistical_tests" in self.results
            and "baseline_comparison" in self.results["statistical_tests"]
        ):
    ...
    def _plot_q1_comparison(self):
        ...
        bars = plt.bar(strategies, disruptions, yerr=stds, capsize=5, color="steelblue", alpha=0.8)
    ...
    def _plot_q2_resilience(self):
        ...
        axes[0].axvline(
            np.mean(resilience_scores), color="red", linestyle="--", linewidth=2, label="Mean"
        )
        ...
        plt.suptitle(
            "Q2: Temporal Resilience Prediction Analysis", fontsize=14, weight="bold", y=1.02
        )
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    model_config = {"num_node_features": 64, "num_edge_features": 32, "hidden_dim": 128}

    # Create and run experiment
    experiment = TemporalGNNExperiment(
        config=network_config, model_config=model_config, output_dir="experiments/results"
    )
    ...
    print("  ✓ 12+ baseline methods compared")
    print("  ✓ Statistical significance established")
    print("  ✓ Component contributions quantified")
